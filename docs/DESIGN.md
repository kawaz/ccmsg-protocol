...
**The page's origin and the instance's endpoint are separate things**, and a credential record keeps both. The `origin` is the site the page that created it was served from, which need not be an endpoint at all: a web UI may be published anywhere and speak to instances elsewhere. **Holding a credential to that one site is this contract's rule rather than WebAuthn's.** A passkey is bound to its `rp_id`, and a relying party may be a suffix of the page's host, so the authenticator by itself would answer for every origin under that suffix. What binds a credential to one site is the check on the record: the `clientDataJSON.origin` of every ceremony, registration and assertion alike, has to equal it. Keeping `rp_id` at the origin's host is what makes the authenticator's own binding say the same single site rather than something wider. A person using two sites holds two credentials, one per site.

That reading is what the origin's other two uses rest on. A token family carries the `origin` of the credential that authenticated, and the WebSocket handshake holds the connection's `Origin` header to it: a token is who the person is and says nothing about what is holding it, so a leaked one presented from any other page is refused — the upgrade does not happen, rather than a connection being made and answered with an error, since the token is the carrier's to read and no frame has been exchanged yet. And the HTTP auth ops answer CORS for the origins an endpoint's credentials name, together with the origin an outstanding registration URL names (`RegisterClaims.origin`), which is what makes a first registration at a new site answerable by the same rule. There is no list to administer: registering is what admits a site and removing the last credential is what drops it. Which instances a page may dial is the page's own declaration and belongs to the web UI rather than to this contract.

The refresh token is an HttpOnly cookie set at `<endpoint>auth/*` and never in a reply's body. Where the page is a site of its own, the cookie is sent across sites only as a partitioned one, which keeps a session taken at one site from being carried to another — the shape the credential's single origin already has. The three ops that settle an identity are held to two headers, the only ones this contract reads over HTTP: the `Origin`, against the origin the credential or the registration URL names, and `Sec-Fetch-Site`, which has to say the call came from a page rather than from a URL someone typed. A header that is missing fails as one that is wrong does, and either is `auth_invalid`, which does not say which. `auth.challenge` is held to neither, having nothing yet to be held to — what it hands out is spendable only at its issuer. **A browser that does not partition cookies sends none across sites**: the refresh fails once the access token expires and the client asserts the passkey again, which costs a user verification and no function. How the cookie's attributes are composed and how the headers are checked are the daemon's, as the rest of the procedure is.

A credential record keeps the `rp_id` it was registered under, which is that origin's host and nothing shorter. A passkey answers only for the domain it was created against, so an assertion's `rpIdHash` is checked against that and not against the host of whatever endpoint was reached.
...
- Identifiers: `sid` is a globally unique uuid, `instance` is the opaque random value an instance issues for itself (16 bytes as hex), `endpoint` is the URL it is dialed at, compared whole including its path, `origin` is the scheme and authority a page was served from, `mid` is `<instance>/<counter>`
