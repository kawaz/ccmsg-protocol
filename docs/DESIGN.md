...
|---|---|---|
| common | everyone | connecting (`hello.session` / `hello.user` / `hello.instance` / `instance.ping` / `instance.shutdown`), declaring the end (`session.stopping`), and subscribing (`topic.subscribe` / `topic.unsubscribe`) |
...
|---|---|---|---|---|
| `whole` | the whole value | replaces everything held | yes | `session.status:<sid>` |
| `per_instance_whole` | the whole of what its `instance` knows | replaces that instance's entries and leaves every other instance's alone (what is held is the union across instances) | yes | `instances`, `session.errors`, `llm.requests`, `llm.status` |
| `element` | the elements that changed | matches on each element's own id and adds or updates; elements it does not mention are untouched, so a removal arrives as a marked element (an absence in a list of changes says nothing) | yes | `peers`, `agents`, `inbox`, `kv:<ns>`, `auth.records` |
...
**A session and a run of it are two things** (DR-0001). The session is the transcript, the folded state and the history: keyed by `instance` and `sid`, it is one thing whether no process or two are running it, and it is a row of `peers`. A run is one process: keyed by `instance` and `pid`, it carries what only a process has — that it is alive, which terminal it is in, whether it is connected — and it is a row of `agents`, before and after the greeting alike. Such a row states a `sid` where it has one; a process a launcher started before the harness had named a session for it has none yet. Connected and lost sessions are still not two lists but one kind of row, so a session starting or going quiet reaches a client as an update to the row it already holds, with its identity intact. Being pinned is a mark a person put there, and travels beside all of this as `pinned`.

**What a person judges on is stated on the row** rather than pressed into one word. `runs` is every process running this session, each with its `pid` and `started_at` (which is what tells a reused pid from the run it was read for), its `terminal_id`, and whether it is connected. `session_status` says what this session's `session.status` fold is worth: `absent` (no transcript yet), `folding`, `ready`, or `frozen`. `stopped_at` says the session declared it was stopping, and says something only while `runs` is empty. `gateway_active_at` says when inference last ran, which is also what makes a session with no run left alive — the request outlives the process. `agents.waiting_for` and `session.status`'s `api_error` say something is out that a person has to answer. The judgements those feed are made by functions the contract exports — `liveness`, `reachable`, `waiting`, against `GATEWAY_LIVE_WINDOW_MS` — so an instance and a client read one row by one rule rather than each writing the arithmetic.

**Two runs of one session are shown, not resolved.** The harness lets a running session be resumed, and from that moment both processes write the same transcript: the reads are untrustworthy and the offsets are wrong. So `session_status` goes `frozen`, the fold stops being updated and the transcript's additions stop being carried, and the last value that could be trusted is what is stated. `message.send`, `notify.send`, `session.dump.write` and the file and directory ops refuse with `session_duplicated` and hold nothing back for later — the draft is still with the caller. `session.kill` takes an optional `pid`, refusing with `ambiguous_run` when there is more than one run and none was named, and signalling only a pid it has found in the session's own runs. Nothing here stops one of the two on its own or decides which is the right one: what it holds to is that the duplicate is not hidden and that a broken reading is never stated as the truth. When the count falls below two, folding starts again from the top — the cached offsets cannot be trusted — and a session with no run left still has its transcript, so it is `folding` and never `absent`.

A run's `terminal_id` is `<scheme>:<id>`, the scheme naming whose handle the id is. `terminalUrl` composes the gateway URL for the `hyoui` scheme and for no other; a handle under another scheme is opened by a client that knows that system, or not at all.

**A terminal is a thing of its own, not a field of a session** (DR-0026). A person opens one with nothing but a shell in it, and it outlives the session that was running there, so the `terminals` topic states the host's terminals as rows in a list of their own, matched by `instance` and `id`. A row carries what the terminal manager sees: its `state`, the `command` running in it, its `cwd`, the `pid` inside it, and when it started. The scheme of the `id` names which manager observed it, so another manager's terminals are rows of the same list. The list is the `user` role's alone — a terminal belongs to the host, and no session has reason to be told which terminal another session is typed into.

Which session is in which terminal is derived from the pids the two lists share, by functions the contract exports: `terminalsOf(sid, agents, terminals)` is where a session's runs are, `unattachedTerminals(agents, terminals)` the terminals no run is in (a person's own shell, and a harness that has started and has not been seen yet), and `starting(terminals, agents)` those of them running a harness that the harness's own list has not caught up with — which is how a run before its state file is said, rather than as an `agents` row with no `sid`. What counts as a harness is the name its command was invoked under (`HARNESS_COMMANDS`), so a shell a person opened is unattached and never starting: nothing there is on its way to becoming a session. `agents.terminal_id` stays for an instance that has no terminal manager to read; where `terminals` is stated, the matching pids are what decide.

`protocol_version` on a `peers` row is the generation the row's own connection announced, so it is absent from a row no client has greeted for: such a row exists because an instance's state file names a session running with no connection at all, not because a client greeted it and was refused.

There is one way in to `stopped_at`: `session.stopping`, by which a session states that it is about to stop, with the instance holding that declaration until the disconnection arrives so that the two are one event in that order. A session that goes without saying so reads as a disappearance — what separates stopping on purpose from being lost is the declaration, not an observation. The session itself calls it (the role is `session` alone), with the harness's end-of-session hook or the `ccmsg` CLI standing in for it.

**How busy a session is is an attribute of the row too**, carried as `gateway_active_at`: when inference last ran for it. A session is busy whether one process or none is running it, so folding the two together would lose one of them. It is an instant rather than a flag because nothing observes the moment a request stops being in flight — a client reads recency against its own threshold. It is absent on an instance with no gateway configured, which means there is nothing observing inference, never that the session is quiet.
...
A type name is `.`-separated, and a prefix names everything below it: `tool` is every tool, `message.user` both directions of what a person and a session said. Three families stay open at their last segment — `tool.<Name>`, `system.attachment.<kind>`, `hook.<Event>` — because the harness coins that segment, and a closed list would turn every newcomer into `unknown` with nothing left to say what arrived. `TRANSCRIPT_ITEM_TYPES` spells out the closed part alone; a name outside it is a newcomer rather than an error. The last segment of an open family is the harness's spelling and not snake_case (`tool.Bash`, `hook.PreToolUse`); it carries no `.` of its own, so a reader may split any type name on `.` and get the hierarchy. `tool.unknown` is this contract's reserved name for a call whose tool the reader could not name — a coined segment like any other, so a harness tool actually called `unknown` lands on it and nothing is lost but the distinction.
...
A credential record keeps the `endpoint` it was registered for, and an assertion is accepted only there: the request has to have arrived at that scheme and authority, with its path below that base URL. `https://h/` and `https://h/personal/` are two endpoints and take two registrations, on one host and one RP ID alike — an RP ID says which domain an authenticator answers for, which is coarser than which instance a person has been admitted to. Binding to the base URL is what stops one instance's credential from being a way into its neighbour.

**Where the web UI is published and where the instance is dialed are separate things**, and a credential record keeps both. The `webui` is the base URL the page that created it was served at — spelled as an endpoint is, path and trailing slash included, since it too is a base URL several of which may share a host. Both are held to a single spelling per place (lowercase host, no default port, punycode), and a `webui` to one more thing besides: that an origin can always be read off it, since a value whose origin could not be derived would break that reading after the record carrying it had been accepted. It need not be an endpoint at all: a web UI may be published anywhere and speak to instances elsewhere. **Holding a credential to that one origin is this contract's rule rather than WebAuthn's.** A passkey is bound to its relying party, which may be a suffix of the page's host, so the authenticator by itself would answer for every origin under that suffix. What binds a credential to one is the check on the record: the `clientDataJSON.origin` of every ceremony, registration and assertion alike, has to equal the origin of this URL. Keeping the relying party at that URL's host is what makes the authenticator's own binding say the same single site rather than something wider. A person using web UIs at two origins holds two credentials, one per origin.

**What is kept and what is compared are different sizes.** The whole URL is kept — it is where a person is sent by a registration URL, what an operator configures, and what they recognise in a list of their own credentials. Every comparison here is of its origin or its host, because a browser writes no path in an `Origin` header or a `clientDataJSON`; two web UIs under one origin are one place to all of it.

**What a header is compared against is the origin of that URL, derived and never stored beside it** (`originOf`): its scheme and authority, which is all a browser writes in an `Origin` or a `clientDataJSON`. The URL is what is kept because it is what a person is sent to and what an operator configures; a second field holding the origin would be one fact in two places, able to disagree. The derivation carries the normalization those comparisons need — a lowercase scheme and host, a port only where it is not the scheme's own, an address literal in its brackets — and this contract states it in one function rather than once per implementation.

The rest rests on that. A token family carries the `webui` of the credential that authenticated, and the WebSocket handshake holds the connection's `Origin` header to the origin of it: a token is who the person is and says nothing about what is holding it, so a leaked one presented from any other page is refused — the upgrade does not happen, rather than a connection being made and answered with an error, since the token is the carrier's to read and no frame has been exchanged yet. **A missing `Origin` is a mismatch**, on the handshake as on the HTTP ops: every gate has to be passed, and a caller with nothing to compare has not passed this one. And the HTTP auth ops answer CORS for the origins of the web UIs an endpoint's credentials name, together with those of the registration URLs that instance issued and still holds. There is no list to administer: registering is what admits a UI and removing the last credential is what drops it, and the outstanding URL is what carries the first registration at a new UI, before any credential names it. A registration URL names the web UI it sends the person to (`RegisterClaims.webui`), that URL being the thing they open, and it names its issuer's own endpoint — the secret and the count of attempts against the six digits are there and nowhere else, so that is where a registration completes. Which instances a page may dial is the page's own declaration and belongs to the web UI rather than to this contract.

The refresh token is an HttpOnly cookie set at `<endpoint>auth/*` and never in a reply's body. Where the page is a site of its own, the cookie is sent across sites only as a partitioned one, which keeps a session taken at one site from being carried to another. That partition is by site where a credential is by origin, so what keeps a session to the one place it was made is the `Origin` held against the family's web UI; the cookie's partition answers for sites and nothing finer. The three ops that settle an identity are held to two headers, the only ones this contract reads over HTTP: the `Origin`, against the origin of the web UI the credential or the registration URL names, and `Sec-Fetch-Site`, which has to be one of `same-origin`, `same-site` or `cross-site` — the three that say a page made the call. Everything else fails: `none`, which is a request with no initiator at all, an absent header, and a value this contract does not know. A header that is missing fails as one that is wrong does, and either is `auth_invalid`, which does not say which. `auth.challenge` is held to neither, having nothing yet to be held to — what it hands out is spendable only at its issuer. **Carrying that cookie across sites takes a browser that partitions cookies, which is a premise of this contract**: one that does not is not an environment this is spoken over, and nothing here is shaped around it. How the cookie's attributes are composed and how the headers are checked are the daemon's, as the rest of the procedure is.

The relying party is derived from the same URL rather than kept beside it (`rpIdOf`, its host). A passkey answers only for the domain it was created against, so an assertion's `rpIdHash` is the SHA-256 of that host and not of whatever endpoint was reached.
...
- Identifiers: `sid` is a globally unique uuid, `instance` is the opaque random value an instance issues for itself (16 bytes as hex), `endpoint` is the URL it is dialed at, compared whole including its path, `webui` is the base URL a page was served at, of which `originOf` derives the origin a browser states, `mid` is `<instance>/<counter>`
...
|---|---|---|
| ops | 48 | common 15 / messaging 4 / control 29 / mesh 0 |
