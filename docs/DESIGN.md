...
That reading is what the origin's other two uses rest on. A token family carries the `origin` of the credential that authenticated, and the WebSocket handshake holds the connection's `Origin` header to it: a token is who the person is and says nothing about what is holding it, so a leaked one presented from any other page is refused. And the HTTP auth ops answer CORS for the origins an endpoint's credentials name, together with the origin an outstanding registration URL names (`RegisterClaims.origin`), which is what makes a first registration at a new site answerable by the same rule. There is no list to administer: registering is what admits a site and removing the last credential is what drops it. Which instances a page may dial is the page's own declaration and belongs to the web UI rather than to this contract.

The refresh token stays what it was: an HttpOnly cookie set at `<endpoint>auth/*` and never in a reply's body. Where the page is a site of its own, the cookie is sent across sites only as a partitioned one, which keeps a session taken at one site from being carried to another — the shape the credential's single origin already has. The three ops a browser reaches there are held to two headers, the only ones this contract reads: the `Origin`, against the origin the credential or the registration URL names, and `Sec-Fetch-Site`, which has to say the call came from a page rather than from a URL someone typed. Either failing is `auth_invalid`, which does not say which. **A browser that does not partition cookies sends none across sites**: the refresh fails once the access token expires and the client asserts the passkey again, which costs a user verification and no function. How the cookie's attributes are composed and how the headers are checked are the daemon's, as the rest of the procedure is.
