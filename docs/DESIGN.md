...
**Where the web UI is published and where the instance is dialed are separate things**, and a credential record keeps both. The `webui` is the base URL the page that created it was served at — spelled as an endpoint is, path and trailing slash included, since it too is a base URL several of which may share a host. It need not be an endpoint at all: a web UI may be published anywhere and speak to instances elsewhere. **Holding a credential to that one place is this contract's rule rather than WebAuthn's.** A passkey is bound to its `rp_id`, and a relying party may be a suffix of the page's host, so the authenticator by itself would answer for every origin under that suffix. What binds a credential to one is the check on the record: the `clientDataJSON.origin` of every ceremony, registration and assertion alike, has to equal the origin of this URL. Keeping `rp_id` at that host is what makes the authenticator's own binding say the same single site rather than something wider. A person using two web UIs holds two credentials, one per UI.
...
The refresh token is an HttpOnly cookie set at `<endpoint>auth/*` and never in a reply's body. Where the page is a site of its own, the cookie is sent across sites only as a partitioned one, which keeps a session taken at one site from being carried to another — the shape one credential per web UI already has. The three ops that settle an identity are held to two headers, the only ones this contract reads over HTTP: the `Origin`, against the origin of the web UI the credential or the registration URL names, and `Sec-Fetch-Site`, which has to say the call came from a page rather than from a URL someone typed. A header that is missing fails as one that is wrong does, and either is `auth_invalid`, which does not say which. `auth.challenge` is held to neither, having nothing yet to be held to — what it hands out is spendable only at its issuer. **A browser that does not partition cookies sends none across sites**: the refresh fails once the access token expires and the client asserts the passkey again, which costs a user verification and no function. How the cookie's attributes are composed and how the headers are checked are the daemon's, as the rest of the procedure is.

A credential record keeps the `rp_id` it was registered under, which is that web UI's host and nothing shorter. A passkey answers only for the domain it was created against, so an assertion's `rpIdHash` is checked against that and not against the host of whatever endpoint was reached.
