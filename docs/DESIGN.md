...
**The page's origin and the instance's endpoint are separate things**, and a credential record keeps both. The `origin` is the site the page that created it was served from, which need not be an endpoint at all: a web UI may be published anywhere and speak to instances elsewhere. It is not a second rule beside the relying party but the same one written down — a passkey is bound to its `rp_id`, a browser runs no ceremony unless the page is under that domain, and `clientDataJSON` settles the origin at every ceremony. Writing it on the record copies a binding WebAuthn already enforces into somewhere an instance can read without a ceremony in hand. A person using two sites holds two credentials, as an authenticator would not answer for the second site with the first site's key.
...
A credential record keeps the `rp_id` it was registered under, a registrable suffix of that origin's host. A passkey answers only for the domain it was created against, so an assertion's `rpIdHash` is checked against that and not against the host of whatever endpoint was reached.
