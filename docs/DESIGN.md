...
Which session is in which terminal is derived from the pids the two lists share, by functions the contract exports: `terminalsOf(sid, agents, terminals)` is where a session's runs are, `unattachedTerminals(agents, terminals)` the terminals no run is in (a person's own shell, and a harness that has started and has not been seen yet), and `starting(terminals, agents)` those of them that hold a process at all — which is how a run before its state file is said, rather than as an `agents` row with no `sid`. `agents.terminal_id` stays for an instance that has no terminal manager to read; where `terminals` is stated, the matching pids are what decide.
