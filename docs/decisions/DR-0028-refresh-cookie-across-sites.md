...
- cookie の属性は、credential の webui と endpoint が **same-site (registrable domain が同じ) か**で決まる。cross-site では **分割された cookie** (top-level site ごとに別々) としてしか渡らず、これは **credential が webui ごとに 1 つという形とそのまま重なる** — 1 つの site から取った session が別の site に持ち越されることは、どちらの層でも起こらない。属性の綴りは daemon の持ち物で、ここには書かない
