...
| [DR-0031](DR-0031-file-read-answers-bytes-in-ranges.md) | ✅ 実装済 | `file.read` は常に base64 のバイト列を答え、`offset` / `length` で範囲を頼む。`truncated` は数に置き換わり、media type は答えない。書き込みの本文も同じ器 |
