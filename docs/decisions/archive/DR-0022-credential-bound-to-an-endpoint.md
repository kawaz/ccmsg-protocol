...
- passkey は作られたドメインにしか答えないので、assertion の `rpIdHash` は **登録が行われた側の host** と比べる (到達した endpoint の host とではなく)。その host が何であるかは [DR-0027](DR-0027-webui-apart-from-endpoint.md) が決める
